...
function aikDevServer(input, flags, console) {
  ...
  return Promise.all(promiseList).then(function (ngrokUrl) {
    return (0, _webpackDevServer2.default)(filename, flags, ngrokUrl).then(function (server) {
    ...
    var server = _results2[0];

    (0, _restart2.default)({ prc: process, server: server, chalk: _chalk2.default });
    return results;
  }).catch(function (err) {
...
function aikBuild(input, flags, console) {
  ...
  return (0, _webpackBuild2.default)(filename, flags, console);
}
